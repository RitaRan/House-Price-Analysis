...
ames_train = ames_train[-c(462,168),]
ames_train= ames_train[,c("MS.SubClass","MS.Zoning","Lot.Area","Lot.Shape","Neighborhood","House.Style","Overall.Qual","Overall.Cond","Exterior.1st","Year.Remod.Add","Mas.Vnr.Area","Exter.Qual","Foundation","Bsmt.Qual","BsmtFin.Type.1","BsmtFin.SF.1","Bsmt.Unf.SF","Heating.QC","X2nd.Flr.SF","X1st.Flr.SF","Full.Bath","Half.Bath","Bedroom.AbvGr","Kitchen.AbvGr","Kitchen.Qual","Fireplace.Qu","Bsmt.Exposure","Garage.Type", "Wood.Deck.SF","price","Year.Built","Open.Porch.SF","Mas.Vnr.Type","Garage.Area","Garage.Finish",'TotalSq','Condition.1',"Central.Air")]
num = ames_train[,sapply(ames_train, is.numeric)]
ggpairs(num[,1:10])


keep_col = c("MS.SubClass","MS.Zoning","Lot.Area","Lot.Shape","Neighborhood","House.Style","Overall.Qual","Overall.Cond","Year.Built","Year.Remod.Add","Roof.Style","Exterior.1st","Exterior.2nd","Mas.Vnr.Type","Mas.Vnr.Area","Exter.Qual","Foundation","Bsmt.Qual","Bsmt.Exposure","BsmtFin.Type.1","BsmtFin.SF.1","Bsmt.Unf.SF","Heating.QC","X1st.Flr.SF","X2nd.Flr.SF","Bsmt.Full.Bath","Full.Bath","Half.Bath","Bedroom.AbvGr","Kitchen.AbvGr","Kitchen.Qual","Fireplaces","Fireplace.Qu","Garage.Type","Garage.Finish","Garage.Area","Wood.Deck.SF", "Open.Porch.SF","price")   
keep_idx = which(colnames(ames_train) %in% keep_col)
ames_train = ames_train[,keep_idx]"House.Style""Exterior.2nd""Mas.Vnr.Type""Mas.Vnr.Area""Bedroom.AbvGr""Fireplaces""Garage.Type","Garage.Finish"
...
model1 = lm(log(price) ~., data = ames_train)
model1 = lm(price ~  log(Lot.Area) + 
    Lot.Shape + Neighborhood + Overall.Qual + Overall.Cond + 
    Year.Built + Year.Remod.Add + Roof.Style + Exterior.1st + 
    Exter.Qual + Foundation + Bsmt.Qual + Bsmt.Exposure + BsmtFin.SF.1 + 
    Bsmt.Unf.SF + Heating.QC + X1st.Flr.SF + X2nd.Flr.SF +  
      Full.Bath + Half.Bath + Kitchen.AbvGr + Kitchen.Qual + Fireplace.Qu + 
    Garage.Area + Wood.Deck.SF + Open.Porch.SF + House.Style + Exterior.2nd + Mas.Vnr.Type + Mas.Vnr.Area + Bedroom.AbvGr + Fireplaces + Garage.Type + Garage.Finish + BsmtFin.Type.1, data=ames_train)
...
model1 = lm(price^(0.25) ~ TotalSq*Neighborhood + poly(Year.Built,3) + Year.Remod.Add +  Garage.Area + Overall.Qual +  log(Lot.Area) + BsmtFin.SF.1 + Overall.Cond + Total.Bsmt.SF + Central.Air + Bsmt.Full.Bath + Screen.Porch + Kitchen.Qual + Exter.Qual + Bldg.Type + Bsmt.Qual + Enclosed.Porch.is*Enclosed.Porch + Garage.Cond + Heating.QC, data=ames_train.train)
Yhat = predict(model1, newdata=ames_train.test, interval = "pred")
Yhat = Yhat^4
rmse(Yhat[,1],ames_train.test$price)
...
#=========
model1 = lm(price^(0.25) ~ TotalSq*Neighborhood + poly(Year.Built,3) + 
              Year.Remod.Add +  Garage.Area + Overall.Qual +  log(Lot.Area) +
              BsmtFin.SF.1 + Overall.Cond + Central.Air + 
              Bsmt.Full.Bath + Screen.Porch + Kitchen.Qual + Exter.Qual + 
              Bldg.Type + Bsmt.Qual + Enclosed.Porch.is*Enclosed.Porch + 
              Garage.Cond + Heating.QC, data=ames_train)
Yhat = predict(model1, newdata=ames_test, interval = "pred")
Yhat = Yhat^4
rmse(Yhat[,1],ames_test$price)
#========
...
model1 = lm(price^(0.25) ~ area + Year.Built^2 + Year.Remod.Add + Garage.Area  + Overall.Qual +  Lot.Area + BsmtFin.SF.1 + Overall.Cond + Total.Bsmt.SF + Central.Air + Screen.Porch + Kitchen.Qual + Exter.Qual + Bldg.Type + Bsmt.Qual + Garage.Area + Enclosed.Porch + Garage.Cond + Neighborhood + Heating.QC, data=ames_train)
...
# [1] 16107.75, 15714.92, 15715.11, 15695.81 15690.05 14967.28 14881.24
...
model1 = lm(log(price) ~ ., data = ames_train)
biggest = formula(lm(log(price) ~ ., data = ames_train))
...
ames_train = clean_data(ames_train)
load("ames_test.Rdata")
ames_test = clean_data(ames_test[,-1])
...
ames_test = rbind(ames_train[1, ] , ames_test)
ames_test = ames_test[-1,]

library(randomForest)

nt = seq(400,800,50)
na = seq(6,12,2)
error = matrix(0,length(na),length(nt))
i = 1 
j = 1
for (i in 1:length(na)) {
  for (j in 1:length(nt)) {
    RF = randomForest(price^0.25~TotalSq + Overall.Qual + area + Garage.Cars + Exter.Qual + X1st.Flr.SF + Neighborhood + Total.Bsmt.SF + Year.Built + Full.Bath + Kitchen.Qual + Garage.Yr.Blt + Garage.Area  +   Lot.Area + BsmtFin.SF.1 + Year.Remod.Add  + Open.Porch.SF + Garage.Type + Bsmt.Qual + X2nd.Flr.SF + Garage.Finish + TotRms.AbvGrd + Fireplaces + Fireplace.Qu + Mas.Vnr.Area + Heating.QC + BsmtFin.Type.1 + Exterior.1st, data = ames_train,replace = TRUE,mtry=na[i],ntree=nt[j])
    Yhat = predict(RF,ames_test)^4
    error[i,j] = rmse(Yhat, ames_test$price)
  }
}
error
plot(nt,error[1,],type = 'l', xlab = "Number of Trees", ylab="Error Rate",col="black")
lines(nt,error[2,],col='red')
title(main="Error Rate of Random Forest", sub="Number of Trees & Number of attributes")
legend("topright",col = c("black","red"), legend = c("m = 3","m = 7"),lty=1,lwd=1)

RF = randomForest(price^0.25~., data = ames_train,replace = TRUE,mtry=na[i],ntree=nt[j])
importance(RF)
varImpPlot(RF)
