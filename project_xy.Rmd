...
ames_train = ames_train %>% mutate(MS.SubClass = factor(MS.SubClass))
...
# delete Lot.Frontage, Garage.Yr.Blt
data = data %>%
  dplyr::select(c(-Lot.Frontage, -Garage.Yr.Blt))
```
...
```{r}
# linear regression
blinear = bas.lm(price~., data = data[,-1], prior = "g-prior",
...
```{r}
blinear = bas.lm(price~., data = train.recoded[,-1], prior = "g-prior",
                      alpha=dim(data)[1], modelprior = uniform(),
                      method = "MCMC", MCMC.iterations = 100000)
# HPM
...
ypred.hpm = predict(blinear, estimater = "BMA", 
                newdata = test.recoded[,-1], predict = TRUE,
                se.fit = TRUE)  
rmse(exp(ypred$fit), exp(data.test$price))
# BPM 
#BPM = predict(blinear, estimator = "BPM")
#BPM$bestmodel
#blinear$namesx[attr(HPM$fit, 'model')+1]
train.recoded =  data %>%
  mutate(Condition.2 = ifelse(Condition.2 == "Norm", "Norm", "Others"))
test.recoded = ames_test %>%
  mutate(Condition.2 = ifelse(Condition.2 == "Norm", "Norm", "Others"),
         Kitchen.Qual = ifelse(as.character(Kitchen.Qual) == "Po",
                               "Fa", as.character(Kitchen.Qual)),
         Heating.QC = ifelse(as.character(Heating.QC) == "Po", 
                             "Fa", as.character(Heating.QC)),
         Electrical = ifelse(as.character(Electrical==""), 
                             "SBrkr", as.character(Electrical)),
         Garage.Qual = ifelse(as.character(Garage.Qual=="Ex"), 
                              "Gd", as.character(Garage.Qual)),
         Garage.Cond = ifelse(as.character(Garage.Cond=="Ex"), 
...
# recode categorical variables into factors
ames_test = ames_test %>% mutate(MS.SubClass = factor(MS.SubClass))
...
ypred2 = predict(blinear2, estimater = "HPM", 
                 newdata = data.test, predict = TRUE)
rmse(exp(ypred2$fit), exp(data.test$price))
ypred3 = predict(blinear3, estimator = "BPM", 
                 newdata = testdata.recoded,  predict = TRUE)
rmse(exp(ypred3$fit), exp(testdata.recoded$price))
ypred4 = predict(blinear4, estimator = "BPM", 
                 newdata = data.test,  predict = TRUE)
