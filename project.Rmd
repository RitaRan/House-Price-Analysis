...
```{r}
residul = Ypred - ames_test$price
qplot(x = Ypred,y = residul)

```
residuals are uniformly distirbuted over 0.
...
# top 10 over underestimate
```{r}
residual = Ypred - ames_test$price
ntest = dim(ames_test)[1]
nleft = ntest - 10

least_over = sort(residual, partial=nleft)[nleft]
test_PID[ which(residual > least_over )]



```

# top 10 under underestimate
```{r}
residual = -Ypred + ames_test$price
ntest = dim(ames_test)[1]
nleft = ntest - 10
