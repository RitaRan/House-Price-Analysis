...
model1 = lm(log(price) ~  log(TotalSq)*(Neighborhood + House.Style + 
              ...
              Condition.1 + log(Lot.Area)*Lot.Shape + factor(Overall.Qual) +
              factor(Overall.Cond) + factor(MS.SubClass)+
              poly(Year.Built,3) + poly(Year.Remod.Add,2) + 
              Bsmt.Exposure + Bsmt.Qual + BsmtFin.Type.1 + BsmtFin.SF.1 +
              Bsmt.Unf.SF+1 +  Garage.Type*(Garage.Area) + Garage.Finish +
              Exterior.1st  + Foundation +Bedroom.AbvGr + Full.Bath + 
              Half.Bath + Kitchen.AbvGr + Fireplace.Qu + Fireplaces  + 
              Lot.Config + Fence + Wood.Deck.SF + log(Open.Porch.SF+1) + 
              log(Screen.Porch+1) + Roof.Style + Mas.Vnr.Type +
              Mas.Vnr.Area, data=ames_train)
```
