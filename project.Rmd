...
I checked four criteria:

Bias:
```{r}
mean(Yhat-ames_test$price)
```

Maximum Deviation: 
```{r}
max(abs(Yhat-ames_test$price))
```

Mean Absolute Deviation:
```{r}
mean(abs(Yhat-ames_test$price))
```
Mean Square Error:
```{r}
sqrt(mean((Yhat-ames_test$price)^2))
```

We do not know how to calculate the coverage for gradient boosting.

2. Evaluation

Since gradient boosing is hard to intepretate, we cannot directly use calculators to obtain house price of first observation in both train and test datasets.Instead, we used R function to calculate them. 

For first observation in the training dataset:
```{r}
...
For first observation in the test dataset:
```{r}
pred.training.xgboost <- predict(house.xgboost, transform_data(ames_test), missing=NA)
Yhat = exp(pred.training.xgboost)
Yhat[1]
