...
```{r}
model1 = lm(log(price) ~  log(TotalSq)*(Neighborhood + House.Style + Exterior.2nd + Heating.QC + Kitchen.Qual) + 
              Condition.1 + 
              log(Lot.Area)*Lot.Shape + 
              factor(Overall.Qual) + factor(Overall.Cond) + factor(MS.SubClass)+
              poly(Year.Built,3) + poly(Year.Remod.Add,2) + 
              Bsmt.Exposure + Bsmt.Qual + Bsmt.Full.Bath + BsmtFin.Type.1 + BsmtFin.SF.1 + Bsmt.Unf.SF  +  
              ...
              Wood.Deck.SF + log(Open.Porch.SF+1) + log(Screen.Porch+1) + Roof.Style +
              Mas.Vnr.Type + Mas.Vnr.Area, data=ames_train)
```
